...
public class BothScrollView extends FrameLayout {
    ...
    public void setScrollView(View top, View left) {
        this.mTopScrollView = top;
        this.mLeftScrollView = left;
    }
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (onBothScrollChangeListener != null)
            onBothScrollChangeListener.scrollChange(l, t, oldl, oldt);
        if (onScrollLoader != null) {
            if (l < 5 && l <= oldl) {
                onScrollLoader.leftLoader();
            } else if (l > width - 5 && l >= oldl) {
                onScrollLoader.rightLoader();
            } else if (l > width / 3 && l < width * 2 / 3) {
                onScrollLoader.centerLoader();
            }
        }
        if (this.mTopScrollView != null)
            this.mTopScrollView.scrollTo(l, t);
        if (this.mLeftScrollView != null)
            this.mLeftScrollView.scrollTo(l, t);


    }

    public interface OnBothScrollChangeListener {
        void scrollChange(int left, int top, int oldLeft,
                          int oldTop);
    }
    ...
    private OnBothScrollChangeListener onBothScrollChangeListener;

    public void setOnBothScrollChangeListener(OnBothScrollChangeListener onBothScrollChangeListener) {
        this.onBothScrollChangeListener = onBothScrollChangeListener;
    }

    private OnScrollLoader onScrollLoader;
    private int width;

    public void setOnScrollLoader(int width, OnScrollLoader onScrollLoader) {
        this.width = width;
